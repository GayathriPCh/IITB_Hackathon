import { useState, useEffect } from 'react';
import { useAccount } from '../components/AccountProvider';
import { Client } from 'xrpl';
import './ListNFT.css';

function convertHexToString(hex) {
  if (!hex) return '';
  let str = '';
  for (let i = 0; i < hex.length; i += 2) {
    str += String.fromCharCode(parseInt(hex.substr(i, 2), 16));
  }
  return str;
}

const ListNFT = () => {
  const { account } = useAccount();
  const [nfts, setNfts] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchNFTs = async () => {
      if (!account) return;

      const client = new Client('wss://s.altnet.rippletest.net:51233');

      try {
        await client.connect();
        const response = await client.request({
          command: 'account_nfts',
          account: account,
        });

        const userNFTs = response.result.account_nfts.map((nft) => {
          console.log('NFT Raw Data:', nft);
          
          let details = {};
          let image = null;
          
          // Parse URI for metadata
          if (nft.URI) {
            try {
              const decodedUri = convertHexToString(nft.URI);
          
              // Check if it's JSON
              if (decodedUri.startsWith('{') && decodedUri.endsWith('}')) {
                const parsedUri = JSON.parse(decodedUri);
                if (parsedUri.image) {
                  image = parsedUri.image.startsWith('ipfs://')
                    ? parsedUri.image.replace('ipfs://', 'https://ipfs.io/ipfs/')
                    : parsedUri.image;
                }
                if (parsedUri.attributes) {
                  details = parsedUri.attributes;
                }
              } else if (decodedUri.startsWith('ipfs://')) {
                // Direct IPFS link
                image = decodedUri.replace('ipfs://', 'https://ipfs.io/ipfs/');
              } else {
                // Fallback for direct URL or unsupported format
                image = decodedUri;
              }
            } catch (error) {
              console.error('Error parsing URI:', error);
              image = convertHexToString(nft.URI); // Fallback to plain string
            }
          }
          
          // Parse memo data for flight details
          if (nft.Memos && nft.Memos.length > 0) {
            try {
              const memoData = nft.Memos[0].Memo.MemoData;
              if (memoData) {
                const decodedMemo = convertHexToString(memoData);
          
                if (decodedMemo.includes('FlightTicket:')) {
                  const jsonStr = decodedMemo.split('FlightTicket:')[1];
                  const parsedMemo = JSON.parse(jsonStr);
                  details = parsedMemo.details || {};
                } else if (decodedMemo.startsWith('{') && decodedMemo.endsWith('}')) {
                  const parsedMemo = JSON.parse(decodedMemo);
                  details = parsedMemo.details || {};
                }
              }
            } catch (error) {
              console.error('Error parsing memo data:', error);
            }
          }
          

          return {
            tokenId: nft.NFTokenID,
            uri: image,
            details: {
              airline: details.airline || 'N/A',
              flightClass: details.flightClass || details.class || 'N/A',
              seatNumber: details.seatNumber || details.seat || 'N/A',
              departure: details.departure || 'N/A',
              arrival: details.arrival || 'N/A',
              date: details.date || 'N/A'
            }
          };
        });

        setNfts(userNFTs);
      } catch (error) {
        console.error('Error fetching NFTs:', error);
      } finally {
        setLoading(false);
        client.disconnect();
      }
    };

    fetchNFTs();
  }, [account]);

  return (
    <div className="list-nft-container">
      <h1>My Flight Tickets</h1>
      
      {loading ? (
        <div className="loading-state">
          <p>Loading your tickets...</p>
        </div>
      ) : nfts.length === 0 ? (
        <div className="empty-state">
          <p>No flight tickets found in your wallet</p>
        </div>
      ) : (
        <div className="tickets-grid">
          {nfts.map((nft) => (
            <div key={nft.tokenId} className="ticket-card">
              <div className="ticket-image">
                {nft.uri ? (
                  <img src={nft.uri} alt="Flight Ticket" />
                ) : (
                  <p>No Image Available</p>
                )}
              </div>
              <div className="ticket-details">
                <h3>{nft.details.airline || 'Flight'} #{nft.details.seatNumber}</h3>
                <div className="ticket-info">
                  <p><span>Airline:</span> {nft.details.airline}</p>
                  <p><span>Class:</span> {nft.details.flightClass}</p>
                  <p><span>Seat:</span> {nft.details.seatNumber}</p>
                  <p><span>Route:</span> {nft.details.departure} → {nft.details.arrival}</p>
                  <p><span>Date:</span> {nft.details.date}</p>
                </div>
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  );
};

export default ListNFT;
